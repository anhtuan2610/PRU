...
public class PlayerAttack : MonoBehaviour
{
    ...
    private Health enemyHealth; // enemy con di bo duoi dat'
    private EnemyFlyHealth enemyFlyHealth;
    ...
    private string EnemyInSight()
    {
        ...
        if(hit.collider != null)
        {
            if (hit.collider.name == "Enemy1")
                enemyHealth = hit.transform.GetComponent<Health>(); // lấy ra đối tượng bị dame

            if (hit.collider.name == "EnemyFly1")
                enemyFlyHealth = hit.transform.GetComponent<EnemyFlyHealth>(); // lấy ra đối tượng bị dame
    ...
    {
        if (EnemyInSight() == "Enemy1")
            enemyHealth.TakeDamage(punchDamege);

        if (EnemyInSight() == "EnemyFly1")
            enemyFlyHealth.TakeDamage(punchDamege);
    }
}
